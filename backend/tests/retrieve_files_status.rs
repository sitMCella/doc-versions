...
#[tokio::test]
async fn retrieve_files_status_returns_500_for_initialized_repository() {
...
#[tokio::test]
async fn retrieve_files_status_on_master_branch_returns_empty_file_status_vector_for_repository_with_master_branch_and_no_files(
) {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    ...
    assert_eq!(
        response,
        "[]".to_string(),
        "The body content should be an empty vector of files status."
    );
}

#[tokio::test]
async fn retrieve_files_status_on_master_branch_returns_file_status_vector_with_one_element_for_repository_with_master_branch_and_one_file(
) {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    ...
    assert_eq!(
        response,
        "[{\"name\":\"README.md\",\"status\":\"current\"}]".to_string(),
        "The body content should be a vector of files status with one element."
    );
}
