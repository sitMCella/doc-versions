...
#[tokio::test]
async fn delete_file_returns_500_for_initialized_repository() {
    ...
    let commit_message = "delete file".to_string();
    let multipart_form = create_multipart_form(&commit_message).await;
...
#[tokio::test]
async fn delete_file_returns_200_for_repository_with_master_branch_and_existent_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
async fn delete_file_creates_new_commit_for_repository_with_master_branch_and_existent_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
async fn delete_file_deletes_file_for_repository_with_master_branch_and_existent_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    cleanup(&configuration_file, &configuration, &workspace_name);
    assert!(files.contains(&file_name) == false);
}
...
async fn create_multipart_form(commit_message: &String) -> Form {
    let commit_message = commit_message.clone();
    return multipart::Form::new().text("commit_message", commit_message);
}
