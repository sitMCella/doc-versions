...
const ULIMIT_OPEN_FILES_HARD: u64 = ULIMIT_OPEN_FILES_SOFT * 2;

#[tokio::test]
async fn update_file_returns_500_for_initialized_repository() {
    ...
    let commit_message = "create file".to_string();
    let mut file_path = PathBuf::from("tests_execution");
...
#[tokio::test]
async fn update_file_returns_404_for_repository_with_master_branch_and_non_existent_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    let branch_name = "master".to_string();
    let file_name = "README.md".to_string();
    let commit_message = "update file".to_string();
    let mut file_path = PathBuf::from("tests_execution");
    file_path.push(format!("{}_README.md", &workspace_name));
    let copy_file_result = copy_test_file(&file_path);
    assert!(copy_file_result.is_ok());
...
#[tokio::test]
async fn update_file_returns_200_for_repository_with_master_branch_and_already_created_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
async fn update_file_creates_new_commit_for_repository_with_master_branch_and_already_created_file()
{
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    ...
    let last_commit = find_last_commit(&repository).unwrap_or_else(|e| {
...
#[tokio::test]
async fn update_file_creates_new_file_for_repository_with_master_branch_and_already_created_file() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    let files = get_files_from_last_commit(&repository).unwrap_or_else(|e| {
        panic!("Error while retrieving the files from last commit: {:?}", e);
    });
    cleanup(&configuration_file, &configuration, &workspace_name);
...
fn cleanup(configuration_file: &PathBuf, configuration: &Settings, workspace_name: &String) {
    ...
    let mut test_file_path = PathBuf::from("tests_execution");
    test_file_path.push(format!("{}_README.md", &workspace_name));
    if test_file_path.exists() {
        _ = fs::remove_file(test_file_path.as_path());
    }
}
...
fn copy_test_file(test_file_path: &PathBuf) -> std::io::Result<u64> {
    std::fs::copy("worspace_README.md", &test_file_path)
}
...
fn get_files_from_last_commit(repo: &Repository) -> Result<Vec<String>, git2::Error> {
    ...
    statuses.iter().for_each(|status_entry| {
        vec.push(status_entry.path().unwrap().to_string());
    });
    Ok(vec)
}
