...
#[tokio::test]
#[serial]
async fn retrieve_workspace_returns_200_on_no_workspaces() {
    ...
    let address = spawn_app(&configuration);
...
#[tokio::test]
#[serial]
async fn retrieve_workspace_returns_empty_vector_on_no_workspaces() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    assert_eq!(
        response,
        "[]".to_string(),
        "The body content should be an empty vector of workspaces."
    );
}

#[tokio::test]
#[serial]
async fn retrieve_workspace_returns_200_on_one_workspace() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
#[serial]
async fn retrieve_workspace_returns_vector_with_one_element_on_one_workspace() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    assert_eq!(
        ...
        "The body content should be a vector with one workspace"
    );
}
...
fn spawn_app(configuration: &Settings) -> String {
