...
#[tokio::test]
async fn change_branch_returns_500_for_initialized_repository() {
...
#[tokio::test]
async fn change_branch_to_non_existent_branch_returns_500_for_repository_with_master_branch() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
async fn change_branch_to_master_branch_returns_200_for_repository_with_master_branch() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
...
#[tokio::test]
async fn change_branch_to_existent_branch_returns_200() {
    let workspace_name = get_workspace_name();
    let configuration_file = configure_test(&workspace_name).unwrap_or_else(|error| {
        panic!("Error while configuring the test: {:?}.", error);
    });
    ...
    let client = reqwest::Client::new();
    let branch_name = "new_branch".to_string();
    let new_branch_result = create_new_branch(&repository, &branch_name);
    assert!(new_branch_result.is_ok());
...
fn create_new_branch(repository: &Repository, branch_name: &String) -> Result<(), git2::Error> {
